...
- Per-GRN chain: (1) Gate 0 location_key → miss silent skip; (2) STN routing if vendor==warehouse → Map row routed_to_stn=1, no PR; (3) status gate `grn_status_id ≥ grn_receipt_trigger_status` else Held-Pre-QC; (4) Deleted (status 4) special handling — already-receipted → Discrepancy, never → quiet skip; (5) resolve PO (po_ref_num primary, ee_po_id fallback) — **if NO ERPNext-created PO resolves → GRN DRIFT: no auto-PR, GRN Map status=Discrepancy, raise Integration Discrepancy (kind="GRN for unknown PO"), `purchase_receipt` empty, GRN data preserved on the Map row. Resolution is FDE-driven, ERPNext-side ONLY: the FDE reviews the drifted GRN and, if it should be received, submits a Purchase Receipt from the stored GRN data via a "Create PR from this GRN" action — a STANDALONE PR with NO purchase_order link by default (FDE may OPTIONALLY link an existing ERPNext PO if one fits). NO PO is ever created or pushed to EE — the integration only PULLS in this direction. FDE may also dismiss the Discrepancy.** Then resolve Supplier (read key), Warehouse (location_key), Items (sku/ean); (6) build PR — received_qty/rejected_qty/accepted_qty per the corrected model; rate=grn_detail_price; tax derivation parity with push; back-refs ecs_easyecom_grn_id/grn_detail_id/po_detail_id; batch/serial/expiry per Item config + §3.3.6 mandatory_*_for_groups overrides; (7) tolerance check cumulative vs original_quantity per purchase_order_detail_id (over/under % settings) → Discrepancy not Failed; (8) PR submit; GRN Map status=Receipted; Sync Record with Line child.
...
- Tests: receipt at status 3; held at status 1/2 then receipted on later poll when 3; STN routing (self-vendor); Gate-0 silent skip; multi-GRN cumulative tolerance; qc_fail split; Deleted-Post-Receipt Discrepancy; po_status drift Discrepancy; idempotent re-pull (same grn_id) no-op; unknown-PO GRN drift (no ERPNext-created PO resolves → NO auto-PR, Discrepancy, purchase_receipt empty); FDE "Create PR from GRN" → standalone PR (no PO link) or optionally-linked PR; dismiss path.
...
4. **Unknown-PO GRN — RESOLVED (2026-05-28): always drift, FDE-driven standalone PR.** A GRN whose PO did not originate in ERPNext (no po_ref_num/ee_po_id resolves to an ERPNext-created PO) is GRN drift: NO auto-PR, GRN Map status=Discrepancy, Integration Discrepancy raised, purchase_receipt empty, GRN data preserved. Resolution is ERPNext-side only and FDE-driven: a "Create PR from this GRN" action on the drifted GRN Map row builds a Purchase Receipt from the stored GRN data (received_qty / qc_fail split / rate / batch), as a STANDALONE PR with NO purchase_order link by default — the FDE may OPTIONALLY link an existing ERPNext PO if one fits. NO PO is ever created or pushed to EE; the integration only PULLS in this direction (you cannot retroactively create-and-push a PO after goods arrived — that is backwards). The FDE may instead dismiss. This supersedes the Stage-3-coded behaviour (PR auto-submitted then Discrepancy) — corrective commit required.
...
## Stage 3 live-verified contract corrections (Harmony, 12 fixes)

These supersede the corresponding assumptions above where they differ. Each was found and fixed during the 5-round live Harmony smoke; the packet text above predates them. Authoritative.

- **`grn_detail_price` is a LINE TOTAL, not a unit price.** Confirmed across 10 real Harmony GRNs spanning 90 days: `sum(line grn_detail_price) == header.total_grn_value` in every sample (the unit-price reading would imply 10×–1000× the real totals). **PR line rate = `grn_detail_price / received_quantity`.** (Supersedes the "rate ← grn_detail_price" lines above.) All 10 samples were single-line; multi-line distribution is unverified — a multi-line GRN smoke + a pinned multi-line test is a follow-up. Commit `26e0c8a`.
- **EE line array wire key is `items`, not `lineItems`.** Both the CreatePurchaseOrder push and the GRN structures use `items`. Commits `e544e52`, `76fcf67`.
- **`inwarded_warehouse_c_id` is a COMPANY id**, not a warehouse/location key. Resolution to Frappe Warehouse/Company still works (the location_key mapping handles it), but the field's semantic is company-level. Commit `76fcf67`.
- **STN routing — code-correct, UNVERIFIED on real data — §10 PREREQUISITE.** The check `vendor_c_id == inwarded_warehouse_c_id` survives the company-id semantic shift (it's still int==int), but **whether it ever fires depends on an unproven assumption**: that Harmony stamps internal-inward GRNs with `vendor_c_id == company_id`. This tenant has no self-GRN sample to confirm it, and no vendor maps to any known company_id. **Before building §10:** FDE triggers a real self-GRN on Harmony (internal inward / opening-stock on a mapped WH), inspects its `vendor_c_id`; if it equals that location's company_id, the check is correct and §10 builds on it; if not, the check must be adjusted before §10. Until then, treat STN routing as code-correct-but-untested-on-real-data. (The original packet reference to "self-vendor 26564" predates the company-id understanding and is not a confirmed self-GRN sample.)
- **`grn_detail_price` tax-variance is computed on received gross, not accepted.** The receipt event is received qty; variance checks the received line value. Commit `f2921d4`.
- **`grn_detail_price` / batch handling:** EE `batch_code` + `expire_date` are captured to custom fields (`ecs_ee_batch_code` / `ecs_ee_expire_date`) even on non-batch Items (preserves the EE-side data without forcing native Batch on a non-batch Item). Native Batch auto-creation + PR Item `batch_no` link only for `has_batch_no` Items. Commit `8c9574c`.
- **PO→PR linkage is the native `purchase_order_item`** (PR Item field), so ERPNext's own `PO.per_received` updates live and completion detection can lean on it. Commit `df8464f`.
- **Idempotency back-ref is `ecs_easyecom_grn_id`** on the PR (not only the GRN Map row), so re-pull is safe even if the Map row is wiped. Commit `07267d5`.
- **`updatePoStatus` body-code 400 must NOT advance `last_pushed_po_status`** — a failed status push stays retryable. Commit `a6655a3`.
- **`grn_status_id` 5 observed** (beyond the documented 1/2/3/4) — handle gracefully (treat unknown/higher status ids as observe-only, don't crash). Part of commit `76fcf67`.
- **Draft-PO resolution skip** — a GRN resolving to a still-draft (un-submitted) ERPNext PO is skipped/deferred rather than linked. Part of commit `76fcf67`.

## Stage 4 carry-ins (from Stage 3 smoke)

- **Test-isolation bleed — Stage 4 ITEM #1, before UI work.** `bench run-tests` writes test-warehouse values to whatever Account is currently enabled. This is the same class of bug as the §8d incident that wiped the live Harmony account. Fix: test setUps scope to a per-test account, not "the enabled one." Highest priority in Stage 4 because UI tests multiply run frequency and blast radius.
- **Address placeholder → precondition+flag.** CreatePurchaseOrder currently falls back to `"Address pending"` when the warehouse address isn't configured. Replace with a precondition check that flags (Flagged-Not-Created style) rather than sending a placeholder — same "refuse, don't placeholder" principle as §8d's HSN handling.
- **Stock Settings prerequisites → §9 onboarding checklist/primer.** `enable_serial_and_batch_no_for_item=1` + `use_serial_batch_fields=1` must be set on the site for batch-tracked items to flow. Belongs in the §9 primer prerequisites, not just code.
- **Multi-GRN partial cumulative tolerance — unit-verified, NOT live-smoked.** Only full receipts were smoked in Stage 3. The §9 closeout test script must flag this as a watch-item for the first real partial receipt on a client.
