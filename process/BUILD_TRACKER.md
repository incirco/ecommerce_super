...
- §8.3: CreateVendor's street field is `street` (NOT `address` as docs/packet said) — discovered via Stage 6 live smoke test follow-up; 8 candidate names probed, only `street` persists round-trip through `getVendors.address.billing.address`. Fixed in EasyEcom-Supplier-Push ruleset (`billing_street` → `street`). **Asymmetric**: UpdateVendor has NO writeable street field — every candidate (flat + nested) returns HTTP 200 "Updated Successfully" but silently no-ops EE-side. Consequence: a post-Create ERPNext-side street edit will surface as drift on next pull (correct behaviour); FDE must either Dismiss the drift or manually correct EE-side, since the API offers no street-update path.

**Next:** §8f closeout — primer (FDE_PRIMER_section_8_supplier), section_8f.md test script, SPEC §8.3 amendments patch (5 items: taxIdentificationNum short form, CreateVendor dual-id return, UpdateVendor response-vs-request vendorId asymmetry, no-push-deactivate, street-field-name + Update-no-op), pre-existing §8d standing failures (2 suites, see Standing items above) before §8 closeout.
