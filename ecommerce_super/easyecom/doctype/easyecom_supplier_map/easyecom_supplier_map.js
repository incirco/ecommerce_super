...
//
// Stage 1 set up the set_query filter and the drift / push button
// fields (depends_on='status==Drift'). Stage 5 wires the click
// handlers + their confirm dialogs. Mirrors §8e EasyEcom Customer
// Map handlers exactly — same UI shape, scoped to Supplier.
        ...
        frappe.confirm(
            __(
                "<b>Dismiss this drift?</b><br><br>" +
                    ...
                    "<b>ignored</b> — the underlying Supplier + Addresses are NOT touched. " +
                    "The map row returns to <b>Mapped</b>.<br><br>" +
                    "If the divergence is intentional (e.g. you renamed supplier_name " +
                    "in ERPNext on purpose), add the field to <b>Excluded Fields</b> below " +
                    "instead — otherwise the next pull will re-flag the same drift."
            ),
            () => {
                frappe.call({
                    method:
                        "ecommerce_super.easyecom.flows.supplier_pull.dismiss_drift",
                    args: {supplier_map_name: frm.doc.name},
                    freeze: true,
    ...
    push_to_ee_action(frm) {
        if (frm.doc.status !== "Drift") {
            frappe.msgprint({
                title: __("Not in Drift"),
                message: __(
                    "This row is in <b>{0}</b> status, not Drift; use the Supplier-form " +
        ...
        if (!frm.doc.erpnext_name) {
            frappe.msgprint({
                ...
                message: __(
                    "This map row has no linked Supplier; create one in ERPNext " +
        ...
        frappe.confirm(
            __(
                "<b>Push ERPNext → EE?</b><br><br>" +
                    ...
                    "via the §8.3 push (overwriting the EE-side divergence). " +
                    "Real EE write.",
