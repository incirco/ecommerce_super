"""Whitelisted endpoint for the §8.2 Customer Master Mode flip.

Mirrors `item_master_mode.flip_to_erpnext_mastered` exactly — same
role-gating, same explicit-confirm contract, same one-way semantics,
same clean-refusal-on-already-flipped behaviour. Independent of the
Item flip: an account can flip Customer master without flipping Item
(or vice versa) — they're two separate switches on the same Account.
...
the customer master; the pull becomes drift-detection only. Once
flipped, EE-side edits to a mapped customer show as Drift rather than
auto-overwriting ERPNext.
...
@frappe.whitelist()
def flip_to_erpnext_mastered_customers(
    account: str, confirm: str | bool = False
) -> dict[str, Any]:
    """Flip customer_master_mode from onboarding → erpnext_mastered.
    ...
    if not roles.intersection(
        ...
        frappe.throw(
            frappe._(
                "Flipping Customer Master Mode requires EasyEcom FDE or "
    ...
    current_mode = frappe.db.get_value(
        "EasyEcom Account", account, "customer_master_mode"
    )
    ...
    frappe.db.set_value(
        ...
        account,
        {
            ...
            "customer_master_flipped_at": now,
        },
    ...
    return {
        ...
        "flipped_at": str(now),
        "message": frappe._(
            "Account {0} Customer master is now in erpnext_mastered mode. "
            "The §8.2 pull is now drift-detection only — EE-side new "
            "customers and edits to mapped customers will flag as Drift "
